# medir_ESQUELETO.py — CASA da Aula 2: mede o round-trip de 100 mensagens.
#
# Antes de rodar, deixe DOIS servidores no ar:
#   - o servidor_eco.py (TCP) na porta 5000  (Aula 2)
#   - o seu eco UDP        na porta 5001
# Depois:  python medir_ESQUELETO.py
#
# A parte TCP ja esta pronta como MODELO. Complete a parte UDP (TODOs).
import socket
import time
...
def medir_udp(porta=5001):
    ...
    inicio = time.perf_counter()
    for _ in range(N):
        # TODO: envie b"x" para (HOST, porta) com sendto(...)
        # TODO: receba a resposta com recvfrom(1024)
        pass
    dt = time.perf_counter() - inicio
    s.close()
    return dt
